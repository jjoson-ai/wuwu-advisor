import { getBlueprintLabelExplanation } from "./blueprint-label-explanations";
...
function formatBaziDayMasterLabel(value: string) {
  ...
  return `${stem.pinyin} (${stem.polarity} ${stem.element}, ${value.trim()})`;
}

function formatBaziPillarLabel(value: string) {
  const normalized = value.trim();
  ...
  const stem = STEM_METADATA[normalized[0] ?? ""];
  const branch = BRANCH_METADATA[normalized[1] ?? ""];
