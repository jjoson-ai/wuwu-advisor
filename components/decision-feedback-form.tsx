...
import {
  ACTED_ON_OPTIONS,
  type ActedOnValue,
  type DecisionGuidanceFeedbackRow,
} from "@/domain/decision/decision-feedback.types";

type DecisionFeedbackFormProps = {
  decisionGuidanceId: string;
  existingFeedback: DecisionGuidanceFeedbackRow | null;
};
...
export function DecisionFeedbackForm({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/submit-decision-feedback", {
        method: "POST",
        headers: {
    ...
    <div className="card stack" style={{ background: "var(--surface-muted)" }}>
      <div className="stack" style={{ gap: "0.35rem" }}>
        ...
        <p className="muted" style={{ margin: 0 }}>
          Rate how useful this decision guidance felt and whether you acted on it.
        </p>
        {existingFeedback !== null ? (
          <p className="muted" style={{ margin: 0 }}>
            Existing feedback loaded for this saved response.
          </p>
        ) : null}
      </div>
      ...
      <form className="stack" onSubmit={handleSubmit}>
        ...
        <label className="stack" style={{ gap: "0.35rem" }}>
          ...
          <textarea
            ...
            placeholder="What felt useful, usable, or off?"
          />
        </label>
